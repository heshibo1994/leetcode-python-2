...
class Soulution:
	...
	def getNearPoint(self,point,relative2d_3d,coor3d):
		...
		if k!=-1:
			...
			print(coor3d[k][0]*0.00748281 +coor3d[k][1]*(-0.182316)+coor3d[k][2]*(-0.105906 )+1 )
...
s = Soulution()
path = "Desk\\"
coor3d = s.read3D_point(path+"points3D.txt")
marker = s.detectMark(path+"detect.txt")

f = open("Desk\\plane.txt",'w')
for i in range(len(marker)):
	relative2d_3d = s.get2d_3d(path+"images.txt",marker[i][0])
	a = s.getNearPoint(marker[i][1],relative2d_3d,coor3d)
	f.write(a)
	a = s.getNearPoint(marker[i][2],relative2d_3d,coor3d)
	f.write(a)
s.writeTxt(path+"relative2d_3d.txt",relative2d_3d,coor3d)
